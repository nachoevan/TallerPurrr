...
- Buscamos evitar duplicación de código. La duplicación hace que los errores y _bugs_ sean más frecuentes. También se hace más difícil modificar el código.
...
- Nuestro objetivo es reemplazar los _for loops_ o estructuras de repetición.
...
- \texttt{purrr} sirve para vectorizar operaciones y por lo tanto, para iterar
...
Queremos extraer las primeras n letras de algunas palabras, con n variable
```{r map2-ejemplo-1, echo=TRUE}
palabras <- c("recorcholis","nacimiento","rosedal","artista","ion")
cantidad_letras <- c(2,1,3,2,2)
map2_chr(palabras,cantidad_letras,~substr(.x,1,.y)) # los argumentos son .x y .y
```
...
La magia empieza cuando nos damos cuenta de que en una lista podemos meter otra lista... entonces las columnas de un tibble pueden ser listas (en general son vectores)
\vspace{0.4 cm}
